...
class User extends Database {
    ...
    /**
     * Registers a new user with the 'freelancer' role.
     */
    public function registerUser($username, $email, $password, $contact_number) {
        ...
        $sql = "INSERT INTO fiverr_clone_users (username, email, password, contact_number, user_role) VALUES (?, ?, ?, ?, 'freelancer')";
        try {
    ...
    /**
     * UPDATED: Logs in a user and saves their 'user_role' to the session.
     */
    public function loginUser($email, $password) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            // THIS IS THE CRITICAL FIX: Save the user's actual role to the session
    ...
    /**
     * Checks if the logged-in user's role is 'fiverr_administrator'.
     */
